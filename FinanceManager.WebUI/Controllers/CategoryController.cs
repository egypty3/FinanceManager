    ...
    public class CategoryController : Controller
    {
        ...
        public ActionResult Edit(Category category)
        {
            if (ModelState.IsValid)
            {
                if (repository.Categories.Any(x => x.Name.Equals(category.Name, StringComparison.CurrentCultureIgnoreCase) &&
                        x.Type.Equals(category.Type)))                    
                {
                    TempData["error"] = string.Format("Category {0} it already exists", category.Name);
                    return RedirectToAction("Index", new { type = category.Type });
                }
                else
                {
                    repository.Save(category);
                    TempData["message"] = string.Format("Category updated {0}", category.Name);
                    return RedirectToAction("Index", new { type = category.Type });
                }
            }
            else
        ...
        public ActionResult Create(Category category)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    repository.Save(category);
                    TempData["message"] = string.Format("Category has been created {0}", category.Name);
        ...
        public ActionResult Delete(int categoryID)
        {
            ...
            {
                TempData["message"] = string.Format("Category removed {0}", deletedCategory.Name);
            }
